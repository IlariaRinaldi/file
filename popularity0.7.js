...
var collection=["ass","rel","vid","vid_grup","vid_rel","debug"];	//ass contiene gli elementi relativi alla popolarita' assoluta
...
function insertId(from_id,to_id,gruppo){
	MongoClient.connect(url, function(err, db) {
		...
		dbo.collection(collection).update(query,{$set:query},{ upsert: true }, function(err, res) {
			if(from_id==null){
				var query={to_id:to_id,gruppo:gruppi[gruppo]};
